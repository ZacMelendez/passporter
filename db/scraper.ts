...
const ASSET_EXTENSIONS = new Set([
  ...
  'pdf', 'woff', 'woff2', 'ttf', 'eot', 'otf', 'mp4', 'webm', 'mp3', 'wav',
]);
...
async function fetchText(url: string, signal?: AbortSignal): Promise<string> {
  const res = await fetch(url, {
    ...
    headers: {
      'User-Agent':
        'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122 Safari/537.36 pass-port-local-tool',
...
export async function findPrivacyPolicyAndEmails(rawUrl: string): Promise<ScrapeResult> {
    ...
    if (mainOrigin) {
      try {
        ...
        return {
          ...
          emails: [...new Set([...result.emails, ...mainResult.emails])],
        };
      } catch {
